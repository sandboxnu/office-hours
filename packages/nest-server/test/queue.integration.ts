...
describe("Queue Integration", () => {
  ...
  describe('GET /queues/:id', () => {
    it('get a queue', async () => {
      let queue = await QueueFactory.create({questions:[await QuestionFactory.create()]});
  ...
  describe('GET /queues/:id/questions', () => {
    it('returns questions in the queue', async () => {
      let queue = await QueueFactory.create({questions:[await QuestionFactory.create({text: 'in queue'})]});
      await QuestionFactory.create({text: 'not in queue'});

      const res = await supertest().get(`/queues/${queue.id}/questions`).expect(200)
      expect(res.body).toMatchSnapshot();
    })
  });
});